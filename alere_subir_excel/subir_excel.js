function mostrarMensaje(mensaje) {
  ...
  class metodo_post{
    static async insertar(llamar_excel_2) {
      ...
      const encabezado = llamar_excel_2.header();
      const encabezadoValido = validarEncabezado(encabezado);
  
      if (stopLoop || stopLoop_proyecto || stopLoop_longitud_data || !encabezadoValido){
          console.log(`El archivo a detenido la carga, no se subira ningun dato a menos que todos los campos esten corregidos`);
          Swal.fire({
                  text:`El archivo excel contiene datos no encontrados en la base de datos o erroneos, no se subira ningun dato a menos que todos los campos esten corregidos. Por favor verifica los datos con la opcion 'Validar excel'.`,
                  icon: 'error',
  ...
  class validar_excel{
      ...
      if(!encabezadoValido){
          mostrarMensaje(`Las columnas del excel no son correctas o no siguen el orden establecido. Corrige los errores señalados o descarga la plantilla de ejemplo con la opcion "Descargar plantilla"`);
      };
  ...
  function validarEncabezado(encabezado) {
    const valoresValidos = ['COORDINADOR', 'MES', 'PROVEEDOR','NOMBRE DEL RECURSO','PUESTO DEL RECURSO',
    ...
    'FECHA INICIO','FECHA FIN','DIAS DEL MES','DIAS DE ASIGNACION','DIAS FESTIVOS',
    'DIAS DE VACACIONES','HORAS TOTALES DEL MES','HORAS POR DIA','HORAS LABORADAS',
    ...
    for (let i = 0; i < encabezado.length; i++) {
      ...
      if (title !== valorEsperado) {
          mostrarMensaje(`Nombre de columna "${title}" no valido. Deberia ser: ${valorEsperado}`);
          ordenCorrecto = false;
      }
    }
    if(ordenCorrecto){
